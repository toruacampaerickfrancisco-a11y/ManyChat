...
async function sendWhatsAppVideoCard(to, { videoUrl, text, buttons = [], footer = '' }) {
  ...
  try {
    ...
    const possiblePaths = [
      path.join(__dirname, 'generated_videos/nikola_bienvenida.mp4'),
      path.join(__dirname, '../Avatar_Torre_CFE_Completo/nikola_bienvenida.mp4'),
      path.join(__dirname, '../frontend/dist/avatar-torre/nikola_bienvenida.mp4'),
      path.join(__dirname, '../frontend/public/avatar-torre/nikola_bienvenida.mp4'),
      path.join(__dirname, '../frontend/dist/avatar-torre/Nikola_primera_version.mp4')
    ];

    let foundVideoBuffer = null;
    for (const p of possiblePaths) {
      if (fs.existsSync(p)) {
        try {
          foundVideoBuffer = fs.readFileSync(p);
          break;
        } catch (e) {}
      }
    }
